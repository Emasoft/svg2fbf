...
- **T12** — per-frame visual diff of the fresh FBF against the committed golden `tests/fixtures/e2e/text_frames/expected.fbf.svg`. Each frame is extracted from BOTH FBFs as a standalone SVG, then compared via `sbb-compare --json` with a per-pixel-channel threshold of 32/256 and an image-wide `diffPercentage` budget of 3 % per frame. Used to be byte-exact, but the system DejaVu glyph table drifts by sub-pixel amounts across fresh `apt-get install fonts-dejavu` runs, making byte-exact non-deterministic. The pixel-level invariant ("the FBF still LOOKS like the golden") tolerates legitimate font-coordinate drift while still flagging real regressions.
...
- `scripts/regen_text_frames_golden.sh` — regenerate `tests/fixtures/e2e/text_frames/expected.fbf.svg` **inside Docker** (mirrors the T13 image: `python:3.12-slim` + `fonts-dejavu` + `fonts-liberation`). Even with T12 now using visual diff, the golden remains platform-pinned: a Linux-generated golden is what the Docker T12 compares against, so the regen still must happen inside Linux to keep the calibrated baseline near 0 % rather than the ~1-2 % cross-platform AA drift floor. Never regenerate this file on the macOS host directly.

The non-text2path byte-exact tests (host pytest `test_e2e_byte_exact`
and Docker E2E byte-exact step) strip the two version-pinned lines
(`<!-- FILE GENERATED BY svg2fbf vN -->` and
`<fbf:generatorVersion>vN</…>`) before comparing — they are runtime
metadata, not part of the wire format the test polices. This means
the non-text2path golden does **not** need regeneration on every
alpha→beta→rc→stable promotion; only on intentional wire-format
changes. T12 (text2path) is no longer byte-exact, so the pin doesn't
apply to the text2path golden.
