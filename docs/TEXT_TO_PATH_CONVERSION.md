...
- `scripts/regen_text_frames_golden.sh` — regenerate `tests/fixtures/e2e/text_frames/expected.fbf.svg` **inside Docker** (mirrors the T13 image: `python:3.12-slim` + `fonts-dejavu` + `fonts-liberation`). The text→path golden is platform-dependent because Linux and macOS DejaVu Sans have different glyph metric tables, so this regen *must* happen on Linux to match what T12 compares against. Never regenerate this file on the macOS host directly.

The byte-exact tests (host pytest `test_e2e_byte_exact` and Docker T12)
strip the two version-pinned lines (`<!-- FILE GENERATED BY svg2fbf vN -->`
and `<fbf:generatorVersion>vN</…>`) before comparing — they are runtime
metadata, not part of the wire format the test polices. This means
golden files do **not** need regeneration on every alpha→beta→rc→stable
promotion; only on intentional wire-format changes.
