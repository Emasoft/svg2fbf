...
The byte-exact tests strip the two version-pinned lines (`<!-- FILE
GENERATED BY svg2fbf vN -->` and `<fbf:generatorVersion>vN</…>`) before
comparing, so a version bump alone never forces a golden regen.
Regenerate goldens *only* when you intentionally change the wire format
(input frames, the CLI invocation, or the conversion logic):

```bash
# The non-text golden (no font-rasterization → host-deterministic):
./scripts/regen_e2e_reference.sh

# The text→path golden — MUST run inside Docker because Linux DejaVu
# Sans and macOS DejaVu Sans produce different glyph <path d="…"> data;
# the script builds the same image as the T13 test and runs svg2fbf
# inside it:
./scripts/regen_text_frames_golden.sh
```

Both scripts ask for interactive confirmation before overwriting.
`REGEN_E2E_CONFIRM=y` / `REGEN_TEXT_GOLDEN_CONFIRM=y` skip the prompt
for scripted use.
