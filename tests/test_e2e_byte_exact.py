...
# svg2fbf embeds its own version twice in every FBF — once in a comment
# header and once in <fbf:generatorVersion>. Those lines are runtime
# metadata (which build produced this file) and have nothing to do with
# the wire format the byte-exact test is policing. Stripping them lets
# one golden serve every alpha/beta/rc/stable branch without pinning
# the test to a specific version, while still catching every other byte
# of drift in the actual output.
_VERSION_MARKERS = (
    b"FILE GENERATED BY svg2fbf",
    b"<fbf:generatorVersion>",
)


def _strip_version_lines(content: bytes) -> bytes:
    return b"\n".join(line for line in content.split(b"\n") if not any(m in line for m in _VERSION_MARKERS))
...
def test_e2e_byte_exact(tmp_path: Path):
    ...
    expected_raw = EXPECTED_OUTPUT.read_bytes()
    actual_raw = actual_output.read_bytes()

    expected_bytes = _strip_version_lines(expected_raw)
    actual_bytes = _strip_version_lines(actual_raw)
