...
cp "$PROJECT_ROOT/scripts/visual_diff_fbf_vs_golden.py" "$WORK_DIR/visual_diff_fbf_vs_golden.py"
...
COPY visual_diff_fbf_vs_golden.py /test/visual_diff_fbf_vs_golden.py
...
run "T11: svg2fbf --text2path bootstraps Bun + converts text→paths" bash -c '
    ...
    # (visual diff vs committed golden FBF) and T13 (visual diff vs
    # input fixture frames).
...
# T12: per-frame VISUAL diff of the text2path output against the
# committed golden FBF. svg2fbf's --text2path mode rasterises <text>
# into glyph <path d="…"> coordinates via fontconfig + the system
# DejaVu glyph table. Two fresh `apt-get install fonts-dejavu` runs
# can produce sub-pixel coordinate drift even on the same architecture
# — making byte-exact comparison fundamentally non-deterministic.
# Visual comparison at the pixel level (sbb-compare diffPercentage <
# 3 %) is the right invariant: "the FBF still LOOKS like the golden".
# On the very first run the golden is missing — we surface a clear
# actionable message so the operator can capture the freshly generated
# FBF as the new golden.
run "T12: text fixture E2E visual diff (text2path mode vs golden, <3% per frame)" bash -c '
    cd /test
    ...
    python3 /test/visual_diff_fbf_vs_golden.py \
        --produced /tmp/text-out/animation.fbf.svg \
        --golden /test/tests/fixtures/e2e/text_frames/expected.fbf.svg \
        --frame-count 5 \
        --pixel-threshold "${T12_PIXEL_THRESHOLD:-32}" \
        --max-diff-pct "${T12_MAX_DIFF_PCT:-3.0}" \
        --workdir /tmp/t12 \
